...
async function obtenerTareas(req, res, next) {
    console.log('mierda entro en obtenerTarea..');
  ...
  console.log('mierda sale de obtenerTarea..');//BORRAR
  console.log(tareas);//BORRRAR
  res.send(tareas);
  res.end();
}
...
async function crearTarea(req, res) {
 ...
 const contenTypeHeader = req.header("Content-Type");
  ...
  console.log('mierda entro en crearTarea..');
...
/**
 *  Modifica una tarea en la base de datos y retorna el objeto creado con un 
 * id válido.
 * 
 * @param {object} req Petición HTTP 
 * @param {object} res Respuesta HTTP
...
async function modificarTarea(req, res, next) {
     ...
     const contenTypeHeader = req.header("Content-Type");
      if (!validContentTypeHeader(contenTypeHeader)) {
        ...
        res.send({
          message: `Invalid Content-Type : ${contenTypeHeader}`,
          details: "This endpoint expected JSON object with attribute `status`"
        });
        res.end();
        return;
      }
    try{
      ...
      const tareaCreada = await tareasLogic.update(req.params.idTarea, modeloACrear);
        console.log('mierda se modifico la tarea en modificarTarea..');
...
/**
 *  Elimina una tarea en la base de datos y retorna el objeto creado con un 
 * id válido.
 * 
 * @param {object} req Petición HTTP 
 * @param {object} res Respuesta HTTP
...
async function eliminarTarea(req, res, next) {
   try{
      ...
      console.log('mierda entro en eliminarTarea..');
      const tareaCreada = await tareasLogic.erase(idTarea);
      console.log('mierda se elimino la tarea en eliminarTarea..');
      console.log(tareaCreada);
      res.statusCode = 200;
